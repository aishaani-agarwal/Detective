...
// ---------- shared Gemini call ----------
async function gemini(model, body, key = GEMINI_KEY) {
  const r = await fetch(
...
const geminiBench = { chat: 0, tts: 0 };
const geminiReady = (kind) => Date.now() >= geminiBench[kind];
function benchGemini(kind, data) {
  let ms = 60 * 60 * 1000; // no retry hint → likely a daily cap → check back in an hour
  const msg = JSON.stringify((data && data.error) || "");
  const m = msg.match(/retry in ([\d.]+)s/i);
  if (m) ms = Math.max(parseFloat(m[1]) * 1000 + 5000, 30_000); // per-minute cap → short bench
  geminiBench[kind] = Date.now() + ms;
  console.log(`ℹ Gemini ${kind} hit its free-tier limit — benched for ${Math.round(ms / 1000)}s. Free backup takes over; the game keeps running.`);
}
...
app.post("/api/speak", async (req, res) => {
  ...
  // Layer 2: Pollinations TTS — free, paced, still a distinct neural voice per character
  try {
    const buf = await speakViaPollinations(text, POLLI_VOICE[suspect.voice.name] || "alloy");
    res.json({ audio: "data:audio/mpeg;base64," + buf.toString("base64") });
  } catch (err2) {
    console.error("speak error (pollinations):", err2.message);
...
// ============================================================
const { buildImageTask, generateToFile, speakViaPollinations, backupChat, POLLI_KEY_SET, GROQ_KEY_SET } = require("./lib-images.js");
