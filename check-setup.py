...
case_files = [f for f in glob.glob(os.path.join(root, "cases", "*.js"))
              if os.path.basename(f) not in ("index.js", "_prompts.js")]
registered = len(re.findall(r'require\("\./[a-z0-9\-]+\.js"\)', creg))
levels = {}
cats = {}
for f in case_files:
    src = open(f, encoding="utf-8").read()
    m = re.search(r'level: "(\w+)"', src);    levels[m.group(1)] = levels.get(m.group(1), 0) + 1 if m else 0
    c = re.search(r'category: "(\w+)"', src); cats[c.group(1)] = cats.get(c.group(1), 0) + 1 if c else 0
